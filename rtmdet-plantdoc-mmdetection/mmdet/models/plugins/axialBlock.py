...
class AxialAttention1(nn.Module):
    def __init__(self, in_planes, out_planes, groups=8, kernel_size=56,
                 stride=1, bias=False, width=False):
        assert (in_planes % groups == 0) and (out_planes % groups == 0)
        super(AxialAttention1, self).__init__()
...
class AxialBlock(nn.Module):
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=4,
        ...
        width = int(inplanes / self.expansion)
        # Both self.conv2 and self.downsample layers downsample the input when stride != 1
        self.conv_down = conv1x1(inplanes, width)
        self.bn1 = norm_layer(width)
        self.hight_block = AxialAttention1(width, width, groups=groups, kernel_size=kernel_size)
        self.width_block = AxialAttention1(width, width, groups=groups, kernel_size=kernel_size, stride=stride, width=True)
